...
namespace GirlyMerely.Infrastructure.Repositories
{
    public class UsersRepository : IDisposable
    {
        ...
        public string ShortLink(string linkOrg)
        {
            string key = Guid.NewGuid().ToString().Replace("-", "").Substring(0, 4);

            while (_context.Links.Any(s => s.ShortLink == key))
            {
                key = Guid.NewGuid().ToString().Replace("-", "").Substring(0, 4);
            }
        ...
        public User CreateUser(User model, string Password)
        {
         ...
         var x=  _userManager.Create(model, Password);
