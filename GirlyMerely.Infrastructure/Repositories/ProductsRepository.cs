...
namespace GirlyMerely.Infrastructure.Repositories
{
    public class ProductsRepository : BaseRepository<Product, MyDbContext>
    {
        ...
        public List<Product> GetFilteredProducts(string searchQuery)
        {
            return _context.Products.Where(p => p.IsDeleted == false && p.Title.ToLower().Trim().Contains(searchQuery.ToLower().Trim())).Include(a => a.ProductGroup).OrderByDescending(a => a.InsertDate).ToList();
        }

        public Product GetProduct(int id)
        {
            var product = _context.Products.Include(p => p.ProductMainFeatures).Include(p => p.ProductFeatureValues).Include(p => p.Brand)
                .Include(p=>p.ProductGroup).FirstOrDefault(p => p.Id == id);
